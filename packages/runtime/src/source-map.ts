...
function variantKey(selection: VariantSelection): string {
  const parts: string[] = [];
  if (selection.mode) parts.push(selection.mode);
  if (selection.theme) parts.push(selection.theme);
  if (selection.breakpoint) parts.push(selection.breakpoint);
  if (selection.state) parts.push(selection.state);
  return parts.join("+") || "base";
}

function uniqueVariantRefs(refs: VariantRef[]): VariantRef[] {
  const seen = new Set<string>();
  const unique: VariantRef[] = [];
...
export function buildVariantRefs(document: ParsedWireSpecDocument): VariantRef[] {
  ...
  const refs: VariantRef[] = [{ key: "base" }];

  const singletonKinds: [keyof VariantSelection, string[]][] = [
    ["mode", modeList],
    ["theme", themeList],
    ...
    for (const theme of themeList.length ? themeList : [undefined]) {
      for (const breakpoint of breakpointList.length ? breakpointList : [undefined]) {
        for (const state of stateList.length ? stateList : [undefined]) {
          ...
          if (breakpoint) selection.breakpoint = breakpoint;
          if (state) selection.state = state;
          refs.push({
...
function walkTargets(
  ...
  semanticPath: string[],
  callback: (target: SourceTarget) => void,
): void {
...
export function buildSourceMap(
  ...
  const seeds = new Map<string, TargetSeed>();
  const documentId =
    document.metadata.id ?? document.root.id ?? screenIdFromNode(document.root);
