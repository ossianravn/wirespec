...
export function mountReviewRuntime(options: ReviewRuntimeOptions): ReviewRuntimeController {
  ...
  window.addEventListener("wirespec.review.targetSelected", onTargetSelected as EventListener);
  ...
  const controller: ReviewRuntimeController = {
    dispose: () => {
      ...
      window.removeEventListener("wirespec.review.targetSelected", onTargetSelected as EventListener);
