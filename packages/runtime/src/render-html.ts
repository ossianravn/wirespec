...
interface RenderMetadata {
  tabPanelToTabId: Map<string, string>;
}
...
const WIDTH_TOKEN_MAP: Record<string, string> = {
  content: "max-content",
  xs: "20rem",
  sm: "26rem",
  md: "40rem",
...
function listProp(node: WireNode, key: string): string[] {
  const value = node.props[key];
...
function domIdForWireId(screenId: string, wireId: string): string {
  return `ws-${screenId}-${wireId}`;
}

function domIdForNode(screenId: string, node: WireNode): string | undefined {
  return node.id ? domIdForWireId(screenId, node.id) : undefined;
}

function controlIdForNode(screenId: string, node: WireNode): string | undefined {
  return node.id ? `ws-input-${screenId}-${node.id}` : undefined;
}

function autoControlId(screenId: string, node: WireNode, suffix: string): string {
...
function classNames(node: WireNode): string {
  ...
  if (boolProp(node, "selected")) parts.push("is-selected");
  if (boolProp(node, "current")) parts.push("is-current");
  if (boolProp(node, "disabled")) parts.push("is-disabled");
  if (boolProp(node, "busy")) parts.push("is-busy");
  if (boolProp(node, "invalid")) parts.push("is-invalid");
  if (!isVisible(node)) parts.push("is-hidden");
  return parts.join(" ");
}

function widthTokenValue(value: string): string {
  return WIDTH_TOKEN_MAP[value] ?? value;
}

function spaceTokenValue(value: string): string {
  return SPACE_TOKEN_MAP[value] ?? value;
}

function widthStyle(property: "width" | "min-width" | "max-width", value: string): string {
...
function layoutStyle(node: WireNode): string {
  ...
  if (width) parts.push(widthStyle("width", width));
  if (min) parts.push(widthStyle("min-width", min));
  if (max) parts.push(widthStyle("max-width", max));
  if (height) parts.push(`min-height:${escapeHtml(height)}`);

  if (direction === "row" || direction === "column") {
    parts.push(`flex-direction:${direction}`);
  }

  if (align === "start") parts.push("align-items:flex-start");
  if (align === "center") parts.push("align-items:center");
  if (align === "end") parts.push("align-items:flex-end");
  if (align === "stretch") parts.push("align-items:stretch");

  if (justify === "start") parts.push("justify-content:flex-start");
  if (justify === "center") parts.push("justify-content:center");
  if (justify === "end") parts.push("justify-content:flex-end");
  if (justify === "between") parts.push("justify-content:space-between");

  if (gap) parts.push(`gap:${spaceTokenValue(gap)}`);
  if (padding) parts.push(`padding:${spaceTokenValue(padding)}`);
...
function dataAttrs(node: WireNode, context: RenderContext): string {
...
function helperMarkup(helperId: string | undefined, helper: string | undefined): string {
  if (!helper) {
    return "";
  }
...
function renderField(node: WireNode, context: RenderContext): string {
...
function renderTextarea(node: WireNode, context: RenderContext): string {
...
function renderSelect(node: WireNode, context: RenderContext): string {
...
function renderCombobox(node: WireNode, context: RenderContext): string {
...
function renderRadioGroup(node: WireNode, context: RenderContext): string {
...
function renderHeading(node: WireNode, context: RenderContext): string {
...
function renderTextLike(node: WireNode, context: RenderContext): string {
  const text =
...
function renderButton(node: WireNode, context: RenderContext): string {
...
function renderLink(node: WireNode, context: RenderContext): string {
...
function renderList(node: WireNode, context: RenderContext): string {
...
function renderBreadcrumbItem(node: WireNode, context: RenderContext): string {
...
function renderBreadcrumbs(node: WireNode, context: RenderContext): string {
...
function renderTabs(node: WireNode, context: RenderContext): string {
  return `<div ${dataAttrs(node, context)} role="tablist">${renderChildren(node, context)}</div>`;
}

function renderTab(node: WireNode, context: RenderContext): string {
...
function renderTabPanel(node: WireNode, context: RenderContext): string {
...
function renderTable(node: WireNode, context: RenderContext): string {
...
function renderTableRow(node: WireNode, context: RenderContext): string {
  return `<tr ${dataAttrs(node, context)}>${renderChildren(node, context)}</tr>`;
}

function renderTableCell(node: WireNode, context: RenderContext): string {
...
function renderPagination(node: WireNode, context: RenderContext): string {
...
function renderStepper(node: WireNode, context: RenderContext): string {
  return `<ol ${dataAttrs(node, context)}>${renderChildren(node, context)}</ol>`;
}

function renderStep(node: WireNode, context: RenderContext): string {
...
function renderEmptyState(node: WireNode, context: RenderContext): string {
...
function renderToken(node: WireNode, context: RenderContext): string {
  const label =
    stringProp(node, "label") ??
    stringProp(node, "title") ??
    stringProp(node, "text") ??
...
function labelledByHeading(node: WireNode, context: RenderContext): string | undefined {
...
function renderNode(node: WireNode, context: RenderContext): string {
  switch (node.kind) {
    case "screen":
    case "component":
    ...
    case "form": {
      ...
      return renderContainer(
        ...
        context,
        "form",
        submit ? `data-ws-submit="${escapeHtml(submit)}"` : "",
      );
    }
    case "dialog": {
      ...
      return renderContainer(
        ...
        context,
        "section",
        `role="dialog" aria-modal="true"${labelledBy ? ` aria-labelledby="${escapeHtml(labelledBy)}"` : ""}`,
      );
    }
    case "drawer": {
      const labelledBy = labelledByHeading(node, context);
...
function collectRenderMetadata(node: WireNode, metadata: RenderMetadata = { tabPanelToTabId: new Map() }): RenderMetadata {
