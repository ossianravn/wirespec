import { escapeHtml, isVisible, nodeLabel, roleForNode, screenIdFromNode } from "./utils.js";
...
function layoutStyle(node) {
    ...
    if (max && sizeMap[max])
        parts.push(`max-width:${sizeMap[max] === "100%" ? "100%" : sizeMap[max] === "32px" ? "32rem" : sizeMap[max] === "24px" ? "28rem" : sizeMap[max] === "16px" ? "20rem" : sizeMap[max]}`);
    if (align === "center")
    ...
    if (gap && sizeMap[gap])
        parts.push(`gap:${sizeMap[gap]}`);
    if (padding && sizeMap[padding])
        parts.push(`padding:${sizeMap[padding]}`);
    if (cols === "2")
        parts.push("grid-template-columns:repeat(2,minmax(0,1fr))");
    if (sticky === "bottom")
        parts.push("position:sticky;bottom:0;background:var(--ws-surface)");
    return parts.join(";");
}
function dataAttrs(node, screenId) {
    const scope = node.kind === "screen" || node.kind === "component"
        ? "screen"
        : node.children.length > 0
            ? "section"
            : "element";
    const targetId = scope === "screen" ? `screen:${screenId}` : `node:${node.id ?? node.kind}`;
    const attrs = [
...
function renderChildren(node, screenId) {
    return node.children.map((child) => renderNode(child, screenId)).join("");
}
function renderField(node, screenId) {
...
function renderCheckboxLike(node, screenId, role) {
...
function renderSelect(node, screenId) {
...
function renderHeading(node, screenId) {
...
function renderTextLike(node, screenId) {
    const text = stringProp(node, "text") ?? nodeLabel(node) ?? "";
    const tag = node.kind === "helper"
        ? "p"
        : node.kind === "alert"
            ? "div"
            : node.kind === "status"
...
function renderButton(node, screenId) {
...
function renderLink(node, screenId) {
...
function renderContainer(node, screenId, tag = "div") {
    return `<${tag} ${dataAttrs(node, screenId)}>${renderChildren(node, screenId)}</${tag}>`;
}
function renderList(node, screenId) {
    const tag = node.kind === "list-item" ? "li" : "ul";
    return `<${tag} ${dataAttrs(node, screenId)}>${renderChildren(node, screenId)}</${tag}>`;
}
function renderNode(node, screenId) {
    switch (node.kind) {
        case "screen":
        case "component":
