import { INTERACTIVE_KINDS, REQUIRED_PROPS_BY_KIND, ROOT_KINDS, STRUCTURAL_CHILD_RULES, isKnownKind, } from "./vocabulary.js";
...
function collectTreeIds(node, ids, diagnostics) {
    if (node.id) {
...
function lintRequiredProps(node, diagnostics) {
...
function lintStructuralRule(node, diagnostics) {
...
function walkNode(node, diagnostics, ids, tabControls, tabPanels, options, context = {}) {
...
function lintVariantTargets(diagnostics, variants, knownIds) {
...
function lintVariantInsertedTrees(diagnostics, variants, ids, options) {
...
function lintTabPairs(diagnostics, tabControls, tabPanels) {
...
export function lintWireSpecDocument(document, options = {}) {
    const diagnostics = [];
    ...
    const knownIds = new Set([...ids.keys(), ...insertedIds]);
