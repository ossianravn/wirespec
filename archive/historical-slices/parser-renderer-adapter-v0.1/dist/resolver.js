...
function resolveVariants(document, selection) {
    ...
    const selected = [];
    const pushMatching = (kind, desired) => {
        if (!desired) {
            return;
        }
...
function removeFromParent(parent, childId) {
    parent.children = parent.children.filter((child) => child.id !== childId);
}
function insertRelative(root, parent, refNode, position, inserted) {
...
export function resolveDocument(document, selection = {}) {
    ...
    for (const variant of orderedVariants) {
        for (const op of variant.ops) {
            ...
            if (removedIds.has(op.target)) {
                throw new WireSpecParseError(`Operation targeted a node that was already removed: ${op.target}`, op.span);
            }
